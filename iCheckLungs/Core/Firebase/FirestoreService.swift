...
final class FirestoreService {
    ...
    func updateUserSubscription(uid: String, plan: String, scansLimit: Int, scansRemaining: Int, periodEnd: Date?) async throws {
        var data: [String: Any] = [
            "plan": plan,
        ...
        if let periodEnd {
            data["subscriptionPeriodEnd"] = Timestamp(date: periodEnd)
        }
        let ref = db.collection(AppConstants.Firestore.usersCollection).document(uid)
        try await ref.setData(data, merge: true)
        logger.info("Updated subscription plan to '\(plan)' for uid: \(uid)")
    }
